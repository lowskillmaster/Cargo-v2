...
from .models import Product, Category, CategorySpecification, ProductSpecification, Review
...
def popular_list(request):
	popular_products = Product.objects.filter(available=True)[:7]
		...
		if 'review_form' in request.POST:
			...
			if review_form.is_valid():
				...
				if existing_review:
					...
					review.is_approved = False  # Требуется модерация
					review.save()
